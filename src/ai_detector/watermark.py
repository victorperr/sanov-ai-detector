...
class GreenlistWatermarker:
    ...
    def _seed(self, context: Sequence[int] | int, position: int) -> int:
        recent = [context] if isinstance(context, int) else list(context)
        recent = recent[-self.context_width:]
    ...
    def greenlist(self, context: Sequence[int] | int, position: int) -> set[int]:
    ...
    def is_green(self, token: int, context: Sequence[int] | int, position: int) -> bool:
    ...
    def generate_toy(self, length: int, *, seed: int = 0, watermarked: bool = True, start_token: int = 0) -> list[int]:
        ...
        for position in range(1, length):
            ...
            candidates = self.greenlist(context, position)
    ...
    def analyze(self, tokens: Sequence[int]) -> WatermarkResult:
        ...
        green_hits = sum(self.is_green(
            token, tokens[:position], position) for position, token in enumerate(tokens[1:], 1))
